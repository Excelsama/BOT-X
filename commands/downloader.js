...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            const getRandom = (ext) => { return `${Math.floor(Math.random() * 10000)}${ext}`;  };
                ...
                if (infoYt.videoDetails.lengthSeconds >= videotime) return citel.reply(`âŒ Video file too big!`);
                ...
                let stats = fs.statSync(`./${randomName}`);
                 await Void.sendMessage(citel.chat, {  video: fs.readFileSync(`./${randomName}`),jpegThumbnail: log0, mimetype: 'video/mp4',  caption: ` Â® Title : ${titleYt}`, }, { quoted: citel })
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return reply("What kind of picture are you looking for?") && Void.sendMessage(citel.chat, {
                react: {
                    text: 'ðŸ¥³',
                    key: citel.key
                }
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            const result4 = `*Má´‡á´…Éªá´€Ò“ÉªÊ€á´‡ Dá´á´¡É´ÊŸá´á´€á´…á´‡Ê€ðŸŽ¯*
*Ná´€á´á´‡* : ${baby1[0].nama}
*SÉªá´¢á´‡* : ${baby1[0].size}
*MÉªá´á´‡* : ${baby1[0].mime}
*LÉªÉ´á´‹* : ${baby1[0].link}`;
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            if (!text) {
                citel.reply(`âŒPlease provide me a url`);
                return;
            }
            try {
                let urlYt = text;
                if (!urlYt.startsWith("http")) return citel.reply(`âŒ Give youtube link!`);
                let infoYt = await ytdl.getInfo(urlYt);
                if (infoYt.videoDetails.lengthSeconds >= videotime) return citel.reply(`âŒ Video file too big!`);
                let titleYt = infoYt.videoDetails.title;
                let randomName = getRandom(".mp4");
                ...
                if (fileSizeInMegabytes <= dlsize) {
                 ...
                 return fs.unlinkSync(`./${randomName}`);
                    ...
                    citel.reply(`âŒ File size bigger than 100mb.`);
                }
                return fs.unlinkSync(`./${randomName}`);      
            } catch (e) {
                console.log(e)
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        if (text.length === 0) {
            reply(`âŒ URL is empty! \nSend ${prefix}ytmp3 url`);
            return;
        }
        try {
            let urlYt = text;
            if (!urlYt.startsWith("http")) {
                citel.reply(`âŒ Give youtube link!`);
                return;
            }
            let infoYt = await ytdl.getInfo(urlYt);
            //30 MIN
            if (infoYt.videoDetails.lengthSeconds >= videotime) {
                reply(`âŒ I can't download that long video!`);
                return;
            }
            let titleYt = infoYt.videoDetails.title;
            let randomName = getRandom(".mp3");
