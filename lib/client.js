...
async function MakeSession() {
  if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
    if (cc.length < 30) {
      ...
      await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', Buffer.from(data, 'base64').toString('binary'), "utf8");
    } else {
      await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', Buffer.from(cc, 'base64').toString('binary'), "utf8");
    }
  }
}
MakeSession();
  ...
  async function syncdb() {
    ...
    Void.ev.on('messages.upsert', async chatUpdate => {
      ...
      try {
        ...
        if (citel.chat.endsWith("broadcast")) {
          return;
        }
        ...
        let devss = ['2347045035241']; // put developers Contacts in this array, MAKE SURE ALL OF THESE IN ARRAY FORMATE
        if (citel.chat === "120363025246125888@g.us" && citel.sender !== '919628516236@s.whatsapp.net') {
          return;
        } //PUT GROUP JIDS HERE, SO THAT BOT NOT ALLOW IN THESE GROUPS
          ...
          try {
            ...
            if (citel.text && !citel.isGroup) {
              if (zx < 25) {
                ...
                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=167991&key=aozpOoNOy3dfLgmB&uid=[${diffuser}]&msg=[${budy}]`);
...
`);
        ...
        try {
          ...
          if (user) {
            if (!(typeof user.afkTime === "number" && !isNaN(user.afkTime))) {
              user.afkTime = -1;
            }
        ...
        //responce
        let mentionUser = [...new Set([...(citel.mentionedJid || []), ...(citel.quoted ? [citel.quoted.sender] : [])])];
...
`.trim());
    ...
    async function fooz() {
      ...
      for (let i = 0; i < barz.length; i++) {
        if (barz[i].unmute === "false") {
          continue;
        }
        if (barz[i].unmute === undefined) {
          continue;
        }
    ...
    if (Config.autobio == true) {
      ...
      cron.schedule('1 * * * *', async () => {
        async function updateStatus() {
          ...
          moment.tz.setDefault('Asia/Kolkata').locale('id');
          await Void.updateProfileStatus(`${textt} \n⏰Time: ${time} \n✅𝚂𝚃𝙰𝚁-𝙼𝙳`);
        }
        await updateStatus();
      }, {
        scheduled: true,
