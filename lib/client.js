...
try {
    ...
    console.log("🌐 Connecting...Please Wait... Succesfully Connected to The Bot Database")
} catch {
    console.log("⚠ Error303... Connection Failed with MongoDb..! Use New MongoDb URI or get Uri from BLADE MD REPOSITORY")
    process.exit(0)
}
...
let cc = Config.sessionName.replace(/BLADE-MD;;;/gi, "").replace(/Secktor;;;/gi, ""); // Dont Remove "Secktor;;;" So that You Can Use Secktor Qr too in your bot
...
`,
                                    ...
                                    try {  await Void.groupParticipantsUpdate(citel.chat, [citel.sender], 'remove')  }
                                     catch { citel.reply('_*Suspecious Link Detected*_\n' + tlang().botAdmin)  }
                                }
                            })
                        }
                    }
...
<html>
  ...
  <body>
    <section>
      Hello from ProfileCorruptedError!
    </section>
  </body>
</html>
`
